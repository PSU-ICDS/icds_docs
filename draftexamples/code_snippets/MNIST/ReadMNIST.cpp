...
//void ReadMNIST( string imageFile, string labelFile, vector< vector<float> > imarr, vector<float> labarr, vector< vector<float> > lab_1hot) {
    ...
    ifstream imfile ( imageFile.c_str(), ios::binary );
    if ( imfile.is_open() ) {
    ...
    ifstream labfile ( labelFile.c_str(), ios::binary );
    if ( labfile.is_open() ) {
