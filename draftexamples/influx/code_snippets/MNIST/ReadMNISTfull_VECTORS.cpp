...
void ReadMNIST( string imageFile, string labelFile, vector< vector<double> > &imarr, vector<double> &labarr, vector< vector<double> > &lab_1hot) {
    ...
    if ( imfile.is_open() ) {
        ...
        imarr.resize( number_of_images, vector<double> (n_rows*n_cols) );
    ...
    if ( labfile.is_open() ) {
        ...
        // Resize image array based on header info
        labarr.resize( number_of_labels );
    ...
    lab_1hot.resize( labarr.size(), vector<double> (10) );
    
    for ( int i = 0; i < lab_1hot.size(); i++ ) {
        for ( int j = 0; j < lab_1hot[1].size(); j++ ) {
...
int main() {
    ...
    vector< vector<double> > train_imar, test_imar;
    vector<double> train_labar, test_labar;
    vector< vector<double> > train_lab1hot, test_lab1hot;
    ...
    srand (time(NULL));
    int randindex = rand() % ( train_labar.size() - 1 );
