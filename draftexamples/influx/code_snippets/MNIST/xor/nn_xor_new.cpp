...
int  main() {
  ...
  int mmax = 4;       // number of input datasets  (4 for XOR problem)
 ...
 int* neuronPerLayer = new int[ numLayers ];
  ...
  float * outputs  = new float[ numRowsData ];
  float ** inputs  = new float*[ numRowsData ];
  ...
  for( int i=0; i<numRowsData; i++)  {             // add the bias input to input array
  ...
  int nnn=0, n=0, m=0;
  float error=0.0, sumDW=0.0, epochError=0.0;
 ...
 for (n = 0; n<nmax; n++) {   //   loop over nmax epochs 
   ...
   epochError =  0.0;
	...
	m = (int)(4 * myRan());          // randomly choose an input
	...
	for ( int i=0; i<neuronPerLayer[0]; i++) {        
	  ...
	  if ( k < (numLayers-1) ) {
        ...
        error = 0.5 * pow( outputs[m] - u[2][0] , 2) ;   // mean square error
      ...
      }  // end of loop over numer of inputs
